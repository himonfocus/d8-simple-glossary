...
class SimpleGlossaryFrontendController extends ControllerBase {
   ...
   */
  public function __construct(StateInterface $state, RequestStack $request, EntityTypeManagerInterface $entity_type_manager, Connection $connection) {
    $this->state = $state;
  ...
  public function fetchAllTermsFromDb($ltr = '') {
    ...
    if ((isset($keyword)) && (!empty($keyword))) {
      ...
      if (($keywordLength == 1) && (preg_match("/^[a-zA-Z]$/", strtoupper($keyword)))) {
        $qry = $this->connection->query($base_query . " WHERE ( SUBSTR(term,1,1) = :d )", [':d' => $keyword]);
      }
      else {
        $qry = $this->connection->query($base_query . " WHERE ( term LIKE :d )", [':d' => '%' . $keyword . '%']);
      }
    }
    elseif ((isset($ltr)) && (!empty($ltr))) {
      $qry = $this->connection->query($base_query . " WHERE ( SUBSTR(term,1,1) = :d )", [':d' => $ltr]);
    }
    else {
      $qry = $this->connection->query($base_query);
    }
    $glossaryResult = [];
    if ($qry) {
